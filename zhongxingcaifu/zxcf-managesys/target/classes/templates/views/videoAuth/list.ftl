...
<title>莱客金融-P2P平台(系统管理平台)</title> <#include "../common/header.ftl"/>
	...
	$(function(){
		...
		$("#loginInfoDisplay").typeahead({
			source:function(query,process){
				//发送请求
				$.get("autocomplete.do?query="+query,function(data){
					if(data.length > 0){
							process(data);
					};
				},'json')
		...
		$("#loginInfoDisplay").change(function(){
			$("#loginInfoValue").val("");
			var item = $("#loginInfoDisplay").typeahead("getActive");
			if(item){
				$("#loginInfoValue").val(item.id);
			}
		});
	});
	</script>
